...
public class schoolDBUtil {
	...
	public static boolean addSubject(String subID, int TID, String subName, String subDesc, int hrs) {
		isSuccess = false;
		...
		try {
			...
			String sql = "INSERT INTO subject VALUES('" + subID + "', '" + TID + "', '" + subName + "', '" + subDesc
					+ "', '" + hrs + "')";
	...
	public static boolean createstudentaccount(String name, int age, String address, int phone, int grade,
		...
		try {
			con = DBConnect.getConnection();
			stmt = con.createStatement();
			String sql = "insert into student values (0, '" + name + "', '" + age + "', '" + address + "', '" + grade
					+ "', '" + phone + "', '" + username + "', '" + password + "', '" + regStatus + "')";
			int rs = stmt.executeUpdate(sql);
